...
- **Rotación al cargar** ([[ADR-008 Rotación al Cargar y Bug de Signo en Botes Sostenidos]]): `_update_held_body` controla `angular_velocity` proporcionalmente (`carry_rotation_rate`/`carry_angular_speed_limit`, mismo patrón que la posición) hacia `camera.basis * _manual_rotation` — la pieza sostenida ahora gira con la cámara. El ghost/soldadura final NO usan la cámara, solo `_manual_rotation` (a propósito, preserva las 24 orientaciones "de cubo" para Fase 3).
- **Bug de signo en `_held_half_extent_toward`** (mismo ADR): era `extent + pos_offset`, correcto es `extent - pos_offset` — invisible con una `LoosePiece` suelta (offset local 0), producía huecos reales al sostener y soldar un BOTE de 2+ piezas.
- **Contacto centrado contra superficies curvas** (mismo ADR): `_refine_contact_for_curved_target()` — contra un objetivo `CylinderShape3D`, la normal radial cruda del rayo (nunca exactamente un eje) se reemplaza por el eje de `_manual_rotation` más parecido, para que el offset use la MISMA cara que realmente va a tocar. El hueco de curvatura (una cara plana no puede abrazar un cilindro) sigue existiendo — es geometría real — pero ahora queda parejo en vez de asimétrico.
